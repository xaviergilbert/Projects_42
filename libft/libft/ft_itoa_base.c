...
/*   Updated: 2019/03/08 04:32:30 by xgilbert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char			*ft_itoa_base(intmax_t nb, intmax_t base)
{
	...
	base > 10 && (ft_vabsolue(nb) % base >= 10) ?
	str[p++] = (ft_vabsolue(nb) % base) - 10 + 'a' : 0;
	!(base > 10 && (ft_vabsolue(nb) % base >= 10)) ?
	str[p++] = (ft_vabsolue(nb) % base) + '0' : 0;
	p == 1 && base > 10 ? str[p++] = '0' : 0;
	base == 10 && nb < 0 ? str[p++] = '-' : 0;
	str[p] = '\0';
